...
if ! python -c 'import caffe2'; then
    ...
    ccache -z
    mkdir build && cd build && cmake -DPYTHON_INCLUDE_DIRS="$(python-config --includes)" -DPYTHON_LIBRARY="$libpython" -DCMAKE_INSTALL_PREFIX:PATH="$c2_install_dir" .. && VERBOSE=1 make -j16 &&  make install
    ccache -s
fi
